	...
	<table width="1060px" align="center" class="cart-table" border="0"
		...
		<tbody>
			...
			<c:forEach var="usercartlog" items="${list }">
				<tr>
					<td width="68"><input id="userid" type="hidden" value="${usercartlog.userId }"><a target="_blank" href="${url }/goods/GoodslistByPage?id=${usercartlog.goodsId}"> <img
							src="${url }/public/images/${usercartlog.goods.goodsImg }" class="img">
					</a></td>
					<td><a target="_blank" href="${url }/goods/GoodslistByPage?id=${usercartlog.goodsId}">${usercartlog.goods.goodsName }</a>
						<p class="stxt"></p></td>
					<td>${usercartlog.goods.sellPrice }</td>
					<td align="center">
						<div class="buy-box">
							<span class="goodsQuantity">${usercartlog.goNum }</span>
						</div>
					</td>
					<td><span class="red"> ￥<label name="amountCount">${usercartlog.goods.sellPrice*usercartlog.goNum }</label>
					</span></td>
		...
		<div class="right-box">
			...
			<p class="btn-box">
				<a class="btn button" href="${url }/usercartlog/towTableList">返回购物车</a> <a id="btnSubmit"
					name="btnSubmit" href="${url}/shoporder/update2?id=${orderId }&&shopState=1" class="btnsubmit">确认提交</a>
			</p>
		</div>
	</div>
</body>
